...
def get_bottle_data(barcode):
    ...
    c = conn.cursor()
    c.execute("SELECT barcode, brand, type, actual_price FROM bottle_data WHERE barcode = ?", (barcode,))
    result = c.fetchone()
    conn.close()
    return result
...
class RecyclingApp:
    ...
    def simulate_scan(self):
        ...
        if barcode:
            ...
            if bottle_data:
                barcode, brand, bottle_type, actual_price = bottle_data
                reward = round(float(actual_price) * 0.25, 2)
    ...
    def generate_qr_code(self, total_reward):
        ...
        qr_window.qr_photo = qr_photo  # Prevent image from being garbage collected
